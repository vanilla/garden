...
class Porter {
   ...
   public function dumpTable($format) {
      ...
      timerStop(arrayTranslate($load, array('count' => 'rows'))); // loading table
   ...
   protected function fixFormats($data) {
      ...
      foreach ($data as $table => $tdef) {
         ...
         touchValue('source', $format, array($table, array()));
         touchValue('destination', $format, $format['destination']);

         // Coax the columns into the proper format.
         $columns = array();
         if (isset($tdef['columns'])) {
            foreach ($def['columns'] as $sourceColumn => $cdef) {
               $cformat = array();
               ...
               touchValue('sourcecolumn', $cformat, $sourceColumn);
               touchValue('type', $cformat, 'varchar(255)');
               touchValue('default', $cformat, null);
